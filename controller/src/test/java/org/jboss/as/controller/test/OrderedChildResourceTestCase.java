...
import static org.jboss.as.controller.descriptions.ModelDescriptionConstants.INDEX;
...
public class OrderedChildResourceTestCase extends AbstractControllerTestBase {
    ...
    public void testOrderedChildrenDescription() throws Exception {
        ...
        Assert.assertTrue(reqProps.hasDefined("attr"));
        Assert.assertTrue(reqProps.hasDefined(INDEX));
    ...
    private static class ParentResourceDefinition extends SimpleResourceDefinition {
        ...
        public ParentResourceDefinition() {
            super(PARENT_MAIN,
                    ...
                    new ModelOnlyAddStepHandler(REQUEST_ATTRIBUTES) {
                        @Override
                        protected Resource createResource(OperationContext context) {
                            Resource resource = Resource.Factory.create(false, CHILD.getKey());
    ...
    private static class OrderedChildResourceDefinition extends SimpleResourceDefinition {
        ...
        public OrderedChildResourceDefinition() {
            super(CHILD, new NonResolvingResourceDescriptionResolver(), new OrderedChildAddHandler(REQUEST_ATTRIBUTES), new ModelOnlyRemoveStepHandler());
        }
    ...
    private static class OrderedChildAddHandler extends AbstractAddStepHandler {
        ...
        protected Resource createResource(OperationContext context, ModelNode operation) {
            if (!operation.hasDefined(INDEX) || operation.get(INDEX).asInt() < 0) {
                return super.createResource(context);
            }
            return context.createResource(operation.get(INDEX).asInt());
        }
