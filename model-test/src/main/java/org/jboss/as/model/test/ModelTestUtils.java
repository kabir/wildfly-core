...
public class ModelTestUtils {
    ...
    public static final String TEMP_DISABLE_STRICT_CHECK = "wildfly.test.temp.disable.strict.check";
    ...
    public static void checkFailedTransformedBootOperations(ModelTestKernelServices<?> mainServices,
        ...
        //TODO WFCORE-1950 remove this system property
        boolean tempDisableStrictCheck = Boolean.getBoolean(TEMP_DISABLE_STRICT_CHECK);
        if (tempDisableStrictCheck) {
        ...
        for (ModelNode op : operations) {
            ...
            checkFailedTransformedAddOperation(mainServices, modelVersion, op, config, legacySteps, tempDisableStrictCheck);
        }
    ...
    private static void checkFailedTransformedAddOperation(
            ...
            ModelNode operation, FailedOperationTransformationConfig config, ModelNode legacySteps,
            boolean tempDisableStrictCheck) throws OperationFailedException {
        ...
        if (config.expectFailed(operation)) {
            ...
            if (config.canCorrectMore(operation)) {
                checkFailedTransformedAddOperation(mainServices, modelVersion, config.correctOperation(operation), config, legacySteps, tempDisableStrictCheck);
            }
